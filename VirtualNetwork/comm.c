...
int pkt_receive(node_t *node, interface_t *interface,
    ...
    printf("msg received %s, on node %s, intf %s\n", pkt, node->node_name, interface->if_name);
    ...
    // entry point of stack from bottom
...
static void* _network_start_pkt_receiver_thread(void* arg) {
    ...
    fd_set active_sock_fd_set, backup_sock_df_set;
    ...
    ITERATE_GLTHREAD_BEGIN(&topo->node_list, curr) {
        ...
        FD_SET(node->udp_sock_fd, &backup_sock_df_set);
        
    } ITERATE_GLTHREAD_END(&topo->node_list, curr)

    while (1) {
        memcpy(&active_sock_fd_set, &backup_sock_df_set, sizeof(fd_set));
        ...
        ITERATE_GLTHREAD_BEGIN(&topo->node_list, curr) {
            ...
            if (FD_ISSET(node->udp_sock_fd, &active_sock_fd_set)) {
                ...
                bytes_recvd = recvfrom(
                    node->udp_sock_fd, (char *)recv_buffer,
